...
router.get("/ensLeaderboard", async (req, res) => {
    try {
        ...
        const ethLeaderboardQuery = (0, kysely_2.sql) `
        SELECT * FROM ens_leaderboard
        OFFSET ${validCursor}
        LIMIT 100;
      `;
        const finalStats = await ethLeaderboardQuery.execute(kysely_1.db);
        ...
        const updateFollowersPromises = rows().map((item) => {
            return (0, node_fetch_1.default)(`https://client.warpcast.com/v2/user?fid=${item.fid}`)
                .then(response => response.json())
                .then(userFidJson => {
                const newItem = { ...item, follower_count: userFidJson.result.user.followerCount };
                return newItem;
            })
                .catch(error => {
